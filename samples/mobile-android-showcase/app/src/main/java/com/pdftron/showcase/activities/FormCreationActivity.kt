...
class FormCreationActivity : FeatureActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        initialToolbarTag = DefaultToolbars.TAG_PREPARE_FORM_TOOLBAR
    ...
    override fun onPostCreate(savedInstanceState: Bundle?) {
        ...
        mPdfViewCtrlTabHostFragment!!.addOnToolbarChangedListener { newToolbar ->
            if (newToolbar == DefaultToolbars.TAG_VIEW_TOOLBAR) {
                button.text = getString(R.string.open_form_toolbar)
            } else {
                button.text = getString(R.string.close_form_toolbar)
            }
        }
    }
    ...
    private fun addControls() {
        button.text = getString(R.string.close_form_toolbar)
        button.setCompoundDrawablesWithIntrinsicBounds(R.drawable.ic_prepare_form, 0, 0, 0)
        ...
        button.setOnClickListener {
            if (mPdfViewCtrlTabHostFragment!!.currentToolbarTag != null) {
                if (mPdfViewCtrlTabHostFragment!!.currentToolbarTag == DefaultToolbars.TAG_VIEW_TOOLBAR) {
                    mPdfViewCtrlTabHostFragment!!.openToolbarWithTag(DefaultToolbars.TAG_PREPARE_FORM_TOOLBAR)
                } else {

...
class FillAndSignActivity : FeatureActivity() {

    override fun onCreate(savedInstanceState: Bundle?) {
        initialToolbarTag = DefaultToolbars.TAG_FILL_AND_SIGN_TOOLBAR
    ...
    override fun onPostCreate(savedInstanceState: Bundle?) {
        ...
        mPdfViewCtrlTabHostFragment!!.addOnToolbarChangedListener { newToolbar ->
            if (newToolbar == DefaultToolbars.TAG_VIEW_TOOLBAR) {
                button.text = getString(R.string.open_fill_sign_toolbar)
            } else {
                button.text = getString(R.string.close_fill_sign_toolbar)
            }
        }
    }
    ...
    private fun addControl() {
        button.text = getString(R.string.close_fill_sign_toolbar)
        button.setCompoundDrawablesWithIntrinsicBounds(R.drawable.ic_fill_and_sign, 0, 0, 0)
        ...
        button.setOnClickListener {
            if (mPdfViewCtrlTabHostFragment!!.currentToolbarTag != null) {
                if (mPdfViewCtrlTabHostFragment!!.currentToolbarTag == DefaultToolbars.TAG_VIEW_TOOLBAR) {
                    mPdfViewCtrlTabHostFragment!!.openToolbarWithTag(DefaultToolbars.TAG_FILL_AND_SIGN_TOOLBAR)
                } else {

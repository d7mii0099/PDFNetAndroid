...
object Utils {
     ...
     * @return a File object
     */
    fun createExternalFile(fileName: String, mFileList: MutableList<String>): File {
        val output = File(MiscellaneousSamplesApplication.instance!!.getExternalFilesDir(null), fileName)
    ...
    @Throws(PDFNetException::class)
    fun getFileName(doc: PDFDoc): String {
        return File(doc.sdfDoc.fileName).name
    }

    @Throws(PDFNetException::class)
    fun getFileName(doc: FDFDoc): String {
        return File(doc.sdfDoc.fileName).name
    }

    @Throws(PDFNetException::class)
    fun getFileName(doc: SDFDoc): String {
        return File(doc.fileName).name
    }
}

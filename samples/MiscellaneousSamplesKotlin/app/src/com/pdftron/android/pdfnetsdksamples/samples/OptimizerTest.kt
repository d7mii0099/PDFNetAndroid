...
class OptimizerTest : PDFNetSample() {
    ...
    override fun run(outputListener: OutputListener?) {
        ...
        val input_filename5 = "newsletter_SaveViewerOptimized.pdf"

        //--------------------------------------------------------------------------------
        // Example 1) Optimize a PDF.
        try {
            val doc = PDFDoc(Utils.getAssetTempFile(PDFNetSample.INPUT_PATH + input_filename)!!.absolutePath)
            doc.initSecurityHandler()
            Optimizer.optimize(doc)
            doc.save(Utils.createExternalFile(input_filename2, mFileList).absolutePath, SDFDoc.SaveMode.LINEARIZED, null)
            doc.close()
        } catch (e: Exception) {
        ...
        try {
            ...
            doc.save(Utils.createExternalFile(input_filename3, mFileList).absolutePath, SDFDoc.SaveMode.LINEARIZED, null)
            doc.close()
        } catch (e: Exception) {
            mOutputListener!!.printError(e.stackTrace)
            return
        ...
        try {
            ...
            doc.save(Utils.createExternalFile(input_filename4, mFileList).absolutePath, SDFDoc.SaveMode.LINEARIZED, null)
            doc.close()
        } catch (e: Exception) {
            mOutputListener!!.printError(e.stackTrace)
            return
        ...
        try {
            val doc = PDFDoc(Utils.getAssetTempFile(PDFNetSample.INPUT_PATH + "TigerText.pdf")!!.absolutePath)
            doc.initSecurityHandler()
            ...
            doc.save(Utils.createExternalFile("TigerText_flatten.pdf", mFileList).absolutePath, SDFDoc.SaveMode.LINEARIZED, null)
            doc.close()
        } catch (e: Exception) {
            mOutputListener!!.printError(e.stackTrace)
            return
        ...
        try {
            ...
            val opts = ViewerOptimizedOptions()
            ...
            doc.saveViewerOptimized(Utils.createExternalFile(input_filename5, mFileList).absolutePath, opts)
            doc.close()
        } catch (e: Exception) {
            mOutputListener!!.printError(e.stackTrace)
            return

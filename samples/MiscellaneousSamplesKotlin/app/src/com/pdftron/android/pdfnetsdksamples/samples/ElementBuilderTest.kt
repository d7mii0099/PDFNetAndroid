...
class ElementBuilderTest : PDFNetSample() {
    ...
    override fun run(outputListener: OutputListener?) {
        ...
        try {
            ...
            // same page.
            var img = Image.create(doc.sdfDoc, Utils.getAssetTempFile(INPUT_PATH + "peppers.jpg")!!.absolutePath)
            element = eb.createImage(img, Matrix2D((img.imageWidth / 2).toDouble(), (-145).toDouble(), 20.0, (img.imageHeight / 2).toDouble(), 200.0, 150.0))
            ...
            val font: Font
            val file = Utils.getAssetTempFile(INPUT_PATH + "font.ttf")
            ...
            while (reader.next().also { element = it } != null) // Read page
            // contents
            {
            ...
            // Embed an external font in the document.
            val font2 = Font.createType1Font(doc, Utils.getAssetTempFile(INPUT_PATH + "Misc-Fixed.pfa")!!.absolutePath)
            while (reader.next().also { element = it } != null) // Read page contents
            {
                if (element!!.type == Element.e_text) {

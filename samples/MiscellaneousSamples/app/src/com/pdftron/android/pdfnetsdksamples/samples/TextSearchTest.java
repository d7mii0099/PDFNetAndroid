...
public class TextSearchTest extends PDFNetSample {
	...
	public void run(OutputListener outputListener) {
        ...
        try {
            PDFDoc doc = new PDFDoc(Utils.getAssetTempFile(INPUT_PATH + "credit card numbers.pdf").getAbsolutePath());
            ...
            while (true) {
                ...
                if (result.getCode() == TextSearchResult.e_found) {
                    if (step == 0) {
                        ...
                        mOutputListener.println(result.getResultStr() + "'s credit card number is: ");
                        ...
                        mOutputListener.println("  " + result.getResultStr());
                        ...
                        hlts.begin(doc);
                        while (hlts.hasNext()) {
                            mOutputListener.println("The current highlight is from page: " + hlts.getCurrentPageNumber());
                            hlts.next();
                        }
                        ...
                        doc.save(Utils.createExternalFile("credit card numbers_linked.pdf", mFileList).getAbsolutePath(), SDFDoc.SaveMode.LINEARIZED, null);
                        break;
                    }

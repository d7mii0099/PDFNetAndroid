...
/**
 * A {@link PdfViewCtrlTabFragment2} that has real-time annotation collaboration functionality.
...
public class CollabViewerTabFragment2 extends PdfViewCtrlTabFragment2 {
    ...
    private static final String TAG = CollabViewerTabFragment2.class.getName();
    ...
    public boolean onSingleTapConfirmed(MotionEvent e) {
        ...
        try {
            ...
            ArrayList<Annot> annots = mPdfViewCtrl.getAnnotationListAt(x, y, x + 1, y + 1);
            int page = mPdfViewCtrl.getPageNumberFromScreenPt((double) x, (double) y);

            // find the first valid annot
    ...
    public boolean onQuickMenuClicked(QuickMenuItem menuItem) {
        boolean handled = super.onQuickMenuClicked(menuItem);
        if (!handled) {
                        ...
                        .subscribe(inputEvent -> {
                            switch (inputEvent.getEventType()) {
                                ...
                                case COMMENT_EDIT_FINISHED: {
                                    ReplyInput replyInput = mReplyUiViewModel.getEditCommentLiveData().getValue();
                                    if (replyInput != null) {
                                        mImpl.editComment(replyInput);
                                    }
                                }
                            }
                        }, throwable -> AnalyticsHandlerAdapter.getInstance().sendException(new RuntimeException(throwable)))
    ...
    private void showReplyComponent() {
        ...
        if (activity != null) {
            if (mReplyUiViewModel != null && selectedAnnot != null) {
                // Set up the view model for UI
                String currentAuthorId = getToolManager().getAuthorId();
                String currentAuthorName = getToolManager().getAuthorName();
                ...
                User user = new User(currentAuthorId, currentAuthorName);
                ...
                showReplyFragment(selectedAnnotId, currentAuthorId, selectedAnnotPageNum);
            } else {
                    ...
                    if (documentEntity != null) {
                        if (!documentEntity.getId().equals(mDocumentId)) {
                            ...
                            mAnnotationViewModel =
                                    ViewModelProviders.of(CollabViewerTabFragment2.this,
        ...
        mAnnotationViewModel.getLastAnnotations().observe(lifecycleOwner, lastAnnotationEntities -> {
            if (lastAnnotationEntities != null && !lastAnnotationEntities.isEmpty()) {
                ...
                for (LastAnnotationEntity lastAnnotationEntity : lastAnnotationEntities) {
                    if (mImpl.mLastXfdfIdsToBeConsumed.contains(lastAnnotationEntity.getId())) {
                        // we've already consumed this ID
                        continue;
                    }
                ...
                mImpl.mLastXfdfIdsToBeConsumed.addAll(ids);
                mDisposables.add(mAnnotationViewModel.consumeLastAnnotations(ids)
                        .subscribeOn(Schedulers.io())
                        .observeOn(AndroidSchedulers.mainThread())
                        .subscribe(() -> {
                                    // success
                                    mImpl.mLastXfdfIdsToBeConsumed.removeAll(ids);
                                }, throwable ->
                                        AnalyticsHandlerAdapter.getInstance()
                                                .sendException(new RuntimeException(throwable))
                        )
                ...
                if (mCollabTabListener != null) {
                    mCollabTabListener.onUpdateUndoRedoState();
                }
            }
        });
    }

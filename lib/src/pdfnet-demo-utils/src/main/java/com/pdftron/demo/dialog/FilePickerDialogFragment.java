...
public class FilePickerDialogFragment extends DialogFragment implements
    ...
    public static Bundle buildBundle(int requestCode, int dialogTitleRes,
        ...
        if (customObject != null) {
            ...
            } else if (customObject instanceof Serializable) {
                args.putSerializable(REQUEST_CUSTOM_SERIALIZABLE_DATA, (Serializable) customObject);
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        Parcelable customParcelable = args.getParcelable(REQUEST_CUSTOM_PARCELABLE_DATA);
        Serializable customSerializable = args.getSerializable(REQUEST_CUSTOM_SERIALIZABLE_DATA);
        String customString = args.getString(REQUEST_CUSTOM_STRING_DATA);
        if (customParcelable != null) {
            mCustomObject = customParcelable;
        } else if (customSerializable != null) {
            mCustomObject = customSerializable;
        } else if (customString != null) {
            mCustomObject = customString;
        }
        ...
        if (Utils.isUsingDocumentTree()) {
            startupFileType = BaseFileInfo.FILE_TYPE_EXTERNAL;
        ...
        if (startupFileType == BaseFileInfo.FILE_TYPE_FOLDER) {
            ...
            AnalyticsHandlerAdapter.getInstance().sendEvent(AnalyticsHandlerAdapter.FILE_PICKER_DIALOG_LOCAL);
            if (Utils.isNullOrEmpty(startupFilePath)) {
                initilizeCurrentLocalFolder(context);
            } else {
            ...
            AnalyticsHandlerAdapter.getInstance().sendEvent(AnalyticsHandlerAdapter.FILE_PICKER_DIALOG_EXTERNAL);
            if (Utils.isNullOrEmpty(startupFilePath)) {
    ...
    private void initilizeCurrentLocalFolder(@NonNull Context context) {
        if (Utils.isUsingDocumentTree()) {
            mCurrentFolder = context.getExternalFilesDir(null);
        } else {
    ...
    public void onStart() {
        ...
        // Send event to EventHandler, used to handle case where permission is not available
    ...
    public Dialog onCreateDialog(Bundle savedInstanceState) {
                        ...
                        public void onClick(DialogInterface dialog, int which) {
                            ...
                            if (mViewMode == VIEW_MODE_LOCAL) {
                                if (mLocalFolderListener != null) {
                                    ...
                                    mLocalFolderListener.onLocalFolderSelected(mRequestCode, mCustomObject, mCurrentFolder);
                                }
                            } else {
                                if (mCurrentExternalFolder == null) {
                                    ...
                                    if (mExternalFolderListener != null) {
                                        ...
                                        mExternalFolderListener.onExternalFolderSelected(mRequestCode,mCustomObject, mCurrentExternalFolder);
                                    }
                                }
                            }
                            dismiss();
        ...
        if (getActivity() != null) {
            ...
            mToolbar = dialogLayout.findViewById(R.id.fragment_file_picker_dialog_toolbar);
            mRecyclerView = dialogLayout.findViewById(R.id.fragment_file_picker_dialog_folder_list);
            mEmptyView = dialogLayout.findViewById(android.R.id.empty);
            mProgressBarLayout = dialogLayout.findViewById(R.id.fragment_file_picker_dialog_progress_bar);
            mFab = dialogLayout.findViewById(R.id.fragment_file_picker_dialog_fab);

            mToolbar.setNavigationIcon(null);
            mToolbar.setNavigationContentDescription(R.string.abc_action_bar_up_description);

            mToolbar.setBackgroundColor(mTheme.breadCrumbBackgroundColor);
            ...
            mToolbar.setNavigationOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    if (mViewMode == VIEW_MODE_LOCAL) {
                        if (mCurrentFolder != null && mCurrentFolder.getParentFile() != null) {
                            mCurrentFolder = mCurrentFolder.getParentFile();
                        }
                    } else {
                        if (mCurrentExternalFolder != null && mCurrentExternalFolder.getParent() != null) {
    ...
    protected void handleItemClick(RecyclerView parent, View view, int position, long id) {
        ...
        if (mViewMode == VIEW_MODE_RECENT || mViewMode == VIEW_MODE_FAVORITE) {
            ...
            if (fileInfo.getType() == BaseFileInfo.FILE_TYPE_FOLDER) {
                if (mDialogMode == DIALOG_MODE_FOLDERS) {
                    ...
                    if (mLocalFolderListener != null) {
                        ...
                        mLocalFolderListener.onLocalFolderSelected(mRequestCode, mCustomObject, fileInfo.getFile());
                    }
                    dismiss();
                    return;
                } else {
                ...
                if (externalFile.isDirectory()) {
                    if (mDialogMode == DIALOG_MODE_FOLDERS) {
                        ...
                        if (mExternalFolderListener != null) {
                            ...
                            mExternalFolderListener.onExternalFolderSelected(mRequestCode, mCustomObject, externalFile);
                        }
                        dismiss();
                        return;
                    } else {
            ...
            if (position < 0 || position >= mExternalFileList.size()) {
    ...
    protected void updateToolbarMenu(Menu menu) {
        if (menu != null) {
            ...
            if (Utils.isUsingDocumentTree()) {
                if (itemFavoriteFiles != null) {
                    itemFavoriteFiles.setVisible(false);
                }
                if (itemRecentFiles != null) {
                    itemRecentFiles.setVisible(false);
                }
            }
        }
    }
    ...
    public boolean onMenuItemClick(MenuItem item) {
        ...
        if (item.getItemId() == R.id.item_recent_files) {
            ...
            AnalyticsHandlerAdapter.getInstance().sendEvent(AnalyticsHandlerAdapter.FILE_PICKER_DIALOG_RECENT);
            if (modeChanged) {
                cancelAllThumbRequests();
            ...
            AnalyticsHandlerAdapter.getInstance().sendEvent(AnalyticsHandlerAdapter.FILE_PICKER_DIALOG_FAVORITE);
            if (modeChanged) {
                cancelAllThumbRequests();
                mRecyclerView.swapAdapter(mRecentAdapter, true);
                Utils.safeNotifyDataSetChanged(mRecentAdapter);
    ...
    public void updateTitle() {
        ...
        switch (mViewMode) {
            ...
            case VIEW_MODE_EXTERNAL:
                title = getString(R.string.sd_card_label);
                subTitle = buildPath(Utils.isUsingDocumentTree() ? "" : title);
                break;
        }
        if (mToolbar != null) {
    ...
    public String buildPath(@NonNull String rootName) {
    ...
    public void onPopulateFolderTaskProgressUpdated(File currentFolder) {
        ...
        if (mToolbar != null) {
            ...
            if (mShowNavButton) {
                ...
                icon.mutate().setColorFilter(mTheme.iconColor, PorterDuff.Mode.SRC_ATOP);
    ...
    static class Theme {
        @ColorInt
        final int breadCrumbBackgroundColor;
        @ColorInt
        final int iconColor;

        Theme(int breadCrumbBackgroundColor, int iconColor) {
            this.breadCrumbBackgroundColor = breadCrumbBackgroundColor;
            this.iconColor = iconColor;
        }

        static Theme fromContext(@NonNull Context context) {
            final TypedArray a = context.obtainStyledAttributes(
                    null, R.styleable.FilePickerDialogTheme, R.attr.pt_file_picker_dialog_style, R.style.PTFilePickerDialogTheme);
            int breadCrumbBackgroundColor = a.getColor(R.styleable.FilePickerDialogTheme_breadCrumbBackgroundColor, context.getResources().getColor(R.color.fragment_file_picker_dialog_current_folder));
            int iconColor = a.getColor(R.styleable.FilePickerDialogTheme_iconColor, context.getResources().getColor(R.color.fragment_file_picker_icon_color));
            a.recycle();
            return new Theme(breadCrumbBackgroundColor, iconColor);
        }
    }
}

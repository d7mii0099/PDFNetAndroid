...
public class ThumbnailWorker implements
    ...
    public void tryLoadImageWithUuid(int position, String filename, String identifier, String iconPath, ImageViewTopCrop imageView) {
        if (Utils.isDoNotRequestThumbFile(filename)) {
            return;
        }
        tryLoadImage(position, identifier, iconPath, imageView, MODE_UUID);
    }

    public void tryLoadImage(final int position, final String identifier, String iconPath, ImageViewTopCrop imageView, int mode) {
        if (Utils.isNullOrEmpty(iconPath)) {

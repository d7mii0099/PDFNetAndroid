...
public class ExternalStorageViewFragment extends FileBrowserViewFragment implements
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        mBinding = FragmentExternalStorageViewBinding.inflate(inflater, container, false);
        mToolbar = mBinding.fragmentToolbar;
        mBreadcrumbBarBinding = mBinding.breadcrumbBar;
        mDialogStorageAccessBinding = mBinding.dialogStorageAccess;
    ...
    public void onViewCreated(View view, Bundle savedInstanceState) {
        ...
        mEmptyView = mDialogStorageAccessBinding.empty;
        mEmptyTextView = mDialogStorageAccessBinding.emptyTextView;
        mEmptyTextView.setBackgroundColor(mTheme.emptyTextBackground);
        mEmptyImageView = mDialogStorageAccessBinding.emptyImage;
            ...
            public boolean onItemLongClick(RecyclerView parent, View view, final int position, long id) {
                ...
                if (mActionMode == null) {
                    ...
                    if (activity instanceof AppCompatActivity) {
                        mActionMode = ((AppCompatActivity) activity).startSupportActionMode(ExternalStorageViewFragment.this);
                    }
                    if (mActionMode != null) {
                        mActionMode.invalidate();
                    }
    ...
    public void bindFilterViewModel(Menu menu) {
        ...
        if (fileMenu != null && context != null) {
            ...
            mFilterViewModel.initialize(PdfViewCtrlSettingsManager.KEY_PREF_SUFFIX_EXTERNAL_FILES,
    ...
    public boolean onActionItemClicked(ActionMode mode, MenuItem item) {
        ...
        if (item.getItemId() == R.id.cab_file_rename) {
            if (mCurrentRoot != null && mCurrentFolder != null) {
                ...
                renameFile(activity, mFileInfoSelectedList.get(0));
            }
            return true;
        }
        if (item.getItemId() == R.id.cab_file_copy) {
            if (mCurrentRoot != null && mCurrentFolder != null) {
                ...
                duplicateFile(activity, mFileInfoSelectedList.get(0));
            }
            return true;
        }
        ...
        if (item.getItemId() == R.id.cab_file_favorite) {
            favoriteFile(mFileInfoSelectedList.get(0));
            finishActionMode();
            return true;
        }
        if (item.getItemId() == R.id.cab_file_share) {
            if (mFileInfoSelectedList.size() > 1) {
                ...
                shareFile(activity, mFileInfoSelectedList.get(0));
                finishActionMode();
            }
            return true;
        }
    ...
    private void createBreadcrumb(Context context, ExternalFileInfo folder, int position) {
        ...
        if (folder != null) { // Non-root breadcrumb
            ...
            String rootText = context.getString(R.string.sd_card_label);
    ...
    protected void reloadDocumentList(boolean reloadRoots) {
        Context context = getContext();
        if (context == null) {
            return;
        }
    ...
    protected void showOriginalEmptyResults() {
        ...
        if (mCurrentRoot == null && mCurrentFolder == null) {
            ...
            LifecycleUtils.runOnResume(this, () -> {
                mBinding.emptyTextView.setVisibility(View.VISIBLE);
                mBinding.emptyTextView.setText(R.string.textview_empty_file_list);
                mEmptyImageView.setVisibility(View.GONE);
            });
        }
    }

    private void showNeedAccessImage() {
        LifecycleUtils.runOnResume(this, () -> {
            mBreadcrumbBarScrollView.setVisibility(View.GONE);
            mEmptyView.setVisibility(View.VISIBLE);
            mEmptyTextView.setText(R.string.q_device_storage_message);
    ...
    public void onExternalFileTrashed(ArrayList<ExternalFileInfo> deletedFiles) {
        onExternalFileDeleted(deletedFiles);
    }
        ...
        public void onPrepareHeaderImage(FileInfoDrawer drawer, ImageViewTopCrop imageView) {
            ...
            if (mSelectedFile.isDirectory()) {
                ...
                if (mSelectedFile.isSecured() || mSelectedFile.isPackage()) {
                    ...
                    mThumbnailWorker.tryLoadImageWithUuid(0,
    ...
    protected void renameFile(Context context, ExternalFileInfo selectedFile) {
        ExternalFileManager.rename(context, selectedFile, ExternalStorageViewFragment.this);
    }

    protected void duplicateFile(Context context, ExternalFileInfo selectedFile) {
        ExternalFileManager.duplicate(context, selectedFile, ExternalStorageViewFragment.this);
    }
    ...
    protected void shareFile(Activity activity, ExternalFileInfo selectedFile) {
        if (mOnPdfFileSharedListener != null) {
            Intent intent = Utils.createGenericShareIntent(activity, selectedFile.getUri());
            mOnPdfFileSharedListener.onPdfFileShared(intent);
        } else {
            Utils.shareGenericFile(activity, selectedFile.getUri());
        }
    }

    protected void deleteFile(Context context, ExternalFileInfo selectedFile) {
        if (mCurrentRoot != null && mCurrentFolder != null) {
            // Deleting non-root level item(s)
            ExternalFileManager.delete(context,
                    new ArrayList<>(Collections.singletonList(selectedFile)),
                    ExternalStorageViewFragment.this);
        } else {
            // Removing a root level item
            ExternalFileManager.removeAccess(context,
                    new ArrayList<>(Collections.singletonList(selectedFile)),
                    ExternalStorageViewFragment.this);
        }
    }

    protected void favoriteFile(ExternalFileInfo selectedFile) {
        int fileType = selectedFile.isDirectory() ? BaseFileInfo.FILE_TYPE_EXTERNAL_FOLDER : BaseFileInfo.FILE_TYPE_EXTERNAL;
        FileInfo fileInfo = new FileInfo(fileType, selectedFile.getAbsolutePath(),
                selectedFile.getFileName(), false, 1);
    ...
    protected void onFileClicked(@NonNull ExternalFileInfo file) {
        Activity activity = getActivity();
        if (activity == null) {
            return;
        }
    ...
    public void onPopulateSdFilesTaskStarted(
        ...
        if (Utils.isNullOrEmpty(mSavedFolderUri) && Utils.isNullOrEmpty(mSavedLeafUri)) {
            if (mBreadcrumbBarLayout != null) {
                if (mBreadcrumbBarLayout.getChildCount() == 0) {
                    rebuildBreadcrumbBar(context, null);
                }
                if (mCurrentFolder != null && mCurrentRoot != null && mBreadcrumbBarScrollView != null) {
    ...
    public void onPopulateSdFilesTaskProgressUpdated(
        ...
        if (savedLeaf != null && mBreadcrumbBarLayout != null) {
            rebuildBreadcrumbBar(context, savedLeaf);
        }
        if (mCurrentFolder != null &&
                mBreadcrumbBarScrollView != null && mBreadcrumbBarLayout != null) {
            setCurrentBreadcrumb(findBreadcrumb(mCurrentFolder));
        }
    ...
    protected void convertHtml() {
        FragmentActivity activity = getActivity();
        if (activity != null) {
            mHtmlConversionComponent.handleWebpageToPDF(activity, mCurrentFolder.getUri());
        }
    }
}

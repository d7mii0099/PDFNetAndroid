...
public class PopulateSdFolderTask extends CustomAsyncTask<Void, PublishState, Void> {
    ...
    private boolean accept(
        ...
        return file != null &&
            ...
            (
                file.isDirectory() ||
                    (mAcceptFiles && isMimeTypeHandled(file.getType().toLowerCase()))
            );

    }

    // TODO: when core can handle TXT and Markdown from filter, remove this
    private boolean isMimeTypeHandled(String mimeType) {
        if (mimeType != null && mimeType.startsWith("text/")) {
            return false;
        }
        return Utils.isMimeTypeHandled(mimeType);
    }

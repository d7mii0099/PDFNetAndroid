...
public class FileInfoDrawerFragment extends Fragment {
    ...
    protected FileInfoDrawerFragment.Listener mListener;
    ...
    // File Object
    protected BaseFileInfo mFileInfo;
    ...
    protected JsonArray mJsonButtons;
    protected JsonObject mJsonContent;
    protected boolean mIsSecured = false;

    private GridLayoutManager mRecyclerLayoutManager;
    protected ContentRecyclerAdapter mRecyclerAdapter;

    protected FileBrowserTheme mFileBrowserTheme;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        mFileBrowserTheme = FileBrowserTheme.fromContext(activity);
        mButtonsMenu = new ActionMenu(activity);
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        mRecyclerLayoutManager.setSpanSizeLookup(new GridLayoutManager.SpanSizeLookup() {
            ...
            public int getSpanSize(int position) {
                switch (mRecyclerAdapter.getItemViewType(position)) {
                    case ContentRecyclerAdapter.VIEW_TYPE_ACTION:
                    case ContentRecyclerAdapter.VIEW_TYPE_BUTTON:
                        // Action and Button items occupy only one span
                        return 1;
                    default:
        ...
        int headerTextColor = mFileBrowserTheme.headerTextColor;
        int iconColor = mFileBrowserTheme.iconColor;
        setRecyclerAdapter(headerTextColor, iconColor);
        ...
        mRecyclerAdapter.setOnItemClickListener(new ItemClickHelper.OnItemClickListener() {
            ...
            public void onItemClick(RecyclerView parent, View view, int position, long id) {
                switch (mRecyclerAdapter.getItemViewType(position)) {
                    case ContentRecyclerAdapter.VIEW_TYPE_ACTION:
                        if (mListener != null) {
                            onActionItemClicked(id);
                        }
                        break;
                    case ContentRecyclerAdapter.VIEW_TYPE_BUTTON:
                        if (mListener != null) {
                            onButtonItemClicked(view, id);
                        }
                        break;
                    default:
                        break;
    ...
    protected void onButtonItemClicked(View view, long id) {
    ...
    public void setFile(BaseFileInfo fileInfo) {
        mFileInfo = fileInfo;
    }
...
//            mLockImageView.setVisibility(visible ? View.VISIBLE : View.GONE);
        ...
        if (mShowButtons) {
            for (int i = 0; i < mButtonsMenu.size(); i++) {
                ...
                if (menuItem.isVisible()) {
                    ...
                    jsonObject.addProperty(ContentRecyclerAdapter.ContentInfoKey.IsAction, false);
                    jsonObject.addProperty(ContentRecyclerAdapter.ContentInfoKey.IsSpacer, false);
    ...
    public void setMainContent(CharSequence content, boolean isSecured) {
        ...
        if (content != null) {
            ...
            mJsonContent.addProperty(ContentRecyclerAdapter.ContentInfoKey.IsAction, false);
            mJsonContent.addProperty(ContentRecyclerAdapter.ContentInfoKey.IsSpacer, false);
        } else {
            mJsonContent = null;
        }

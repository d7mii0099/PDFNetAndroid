...
public class SearchToolbar extends Toolbar {
    ...
    public SearchToolbar(Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, R.attr.search_toolbar);
    }

    public SearchToolbar(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        init(context, attrs, defStyleAttr, R.style.SearchToolbarStyle);
    }

    private void init(@NonNull Context context, @Nullable AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        boolean listAllVisible;
        boolean matchCaseVisible;
        boolean wholeWordVisible;
        boolean searchWebVisible;

        TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.SearchToolbar, defStyleAttr, defStyleRes);
        try {
            listAllVisible = typedArray.getBoolean(R.styleable.SearchToolbar_showListAll, true);
            matchCaseVisible = typedArray.getBoolean(R.styleable.SearchToolbar_showMatchCase, true);
            wholeWordVisible = typedArray.getBoolean(R.styleable.SearchToolbar_showWholeWord, true);
            searchWebVisible = typedArray.getBoolean(R.styleable.SearchToolbar_showSearchWeb, true);
        } finally {
            typedArray.recycle();
        }
        ...
        if (menu != null) {
            ...
            if (mMenuListAll != null) {
                ...
                mMenuListAll.setVisible(listAllVisible);
            }
            mMenuMatchCase = menu.findItem(R.id.action_match_case);
            if (mMenuMatchCase != null) {
                mMenuMatchCase.setVisible(matchCaseVisible);
            }
            mMenuWholeWord = menu.findItem(R.id.action_whole_word);
            if (mMenuWholeWord != null) {
                mMenuWholeWord.setVisible(wholeWordVisible);
            }
            mMenuSearchWeb = menu.findItem(R.id.action_search_web);
            if (mMenuSearchWeb != null) {
                mMenuSearchWeb.setVisible(searchWebVisible);
            }

            mMenuProgress = menu.findItem(R.id.search_progress);

            if (mMenuProgress != null && mMenuProgress.getActionView() != null) {
                ProgressBar mProgressBar = mMenuProgress.getActionView().findViewById(R.id.search_progress_spinner);
                if (mProgressBar != null) {
                    if (Utils.isLollipop()) {
                        mProgressBar.setIndeterminateTintList(MenuItemCompat.getIconTintList(mMenuSearchWeb));
                    } else {
                        Drawable drawableProgress = DrawableCompat.wrap(mProgressBar.getIndeterminateDrawable());
                        int menuItemColor = MenuItemCompat.getIconTintList(mMenuSearchWeb).getDefaultColor();
                        DrawableCompat.setTint(drawableProgress, menuItemColor);
                ...
                if (id == R.id.action_search_web) {
                    if (mSearchView != null && mSearchView.getQuery() != null) {
                        ...
                        if (!Utils.isNullOrEmpty(query)) {
                            ...
                            // Reference: https://cketti.de/2020/09/03/avoid-intent-resolveactivity/
                            try {
        ...
        if (Utils.isTablet(getContext()) && !Utils.isRtlLayout(getContext())) {
            ...
            LayoutParams params = new LayoutParams(Gravity.END);

...
/**
 * This class implements <code>TabLayout</code> where each tab is attached with a fragment
 */
public class CustomFragmentTabLayout extends TabLayout
    ...
    public interface OnTabModificationListener<T extends TabLayout.Tab> {
        void onTabAdded(T tab);
        void onTabRemoved(T tab);
    }
    ...
    private final ArrayList<TabLayout.BaseOnTabSelectedListener> mSelectedListeners = new ArrayList<>();

    private OnTabModificationListener mOnTabModificationListener;
    ...
    public void setOnTabModificationListener(@NonNull OnTabModificationListener listener) {
        mOnTabModificationListener = listener;
    }
    ...
    public void addTab(@NonNull Tab tab, Class<?> _class, Bundle args) {
        ...
        if (mOnTabModificationListener != null) {
            mOnTabModificationListener.onTabAdded(tab);
        }
    }
            ...
            if (newTabInfo.mFragment == null) {
                ...
                for (Fragment fragment : mFragmentManager.getFragments()) {
                    if (fragment instanceof PdfViewCtrlTabBaseFragment) {
                        String tabTag = ((PdfViewCtrlTabBaseFragment) fragment).getTabTag();
    ...
    public void removeTab(Tab tab) {
        ...
        if (mOnTabModificationListener != null) {
            mOnTabModificationListener.onTabRemoved(tab);
        }
    }
    ...
    public Fragment getFragmentByTag(String tag) {
        if (tag != null) {
            for (int i = 0, sz = mTabs.size(); i < sz; i++) {
                TabInfo tabInfo = mTabs.get(i);
                if (tabInfo.mTag.equals(tag) && tabInfo.mFragment != null) {

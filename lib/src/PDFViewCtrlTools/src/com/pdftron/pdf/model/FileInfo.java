...
public class FileInfo implements BaseFileInfo {
    ...
    private boolean mIsTodayHeading = false;
    private boolean mIsThisWeekHeading = false;
    private boolean mIsEarlierHeading = false;

    public static final int HEADER_TYPE_TODAY = 0x00;
    public static final int HEADER_TYPE_THIS_WEEK = 0x01;
    public static final int HEADER_TYPE_EARLIER = 0x02;
    ...
    public FileInfo(int type, String headerText, int headerType) {
        ...
        mIsHeader = true;
        switch (headerType) {
            case HEADER_TYPE_TODAY: {
                mIsTodayHeading = true;
                break;
            }
            case HEADER_TYPE_THIS_WEEK: {
                mIsThisWeekHeading = true;
                break;
            }
            default: {
                mIsEarlierHeading = true;
                break;
            }
        }
    }
        ...
        try {
            ...
            if (jsonObject.has(VAR_FILE)) {
                ...
                try {
                    ...
                    // try backup loading
                    String absolutePath = fileObject.optString(VAR_PATH);
            ...
            if (jsonObject.has(VAR_HEADER_TEXT)) {
                mSectionFirstPos = jsonObject.getInt(VAR_HEADER_TEXT);
            }
        } catch (Exception e) {
    ...
    @Override
    public long getSize() {
        return mFile == null ? 0L : mFile.length();
    }
    ...
    public boolean equals(Object o) {
        ...
        try {
            ...
            sendException(that.getAbsolutePath(), that.getType());
            return false;
        }
    }
    ...
    private void sendException(
        ...
        AnalyticsHandlerAdapter.getInstance().sendException(new Exception("filepath:" + path + ", type:" + type));
    }
    ...
    public boolean isTodayHeading() {
        return mIsTodayHeading;
    }

    @Override
    public boolean isThisWeekHeading() {
        return mIsThisWeekHeading;
    }

    @Override
    public boolean isEarlierHeading() {
        return mIsEarlierHeading;
    }
}

...
public class TextHighlighter extends Tool implements GenerateHighlightsTask.Callback {
    ...
    public void setHighlightColors(int highlightColorInDayMode, int highlightColorInNightMode, int selColorInDayMode, int selColorInNightMode) {
        ...
        mSelectionColor = isNightMode ? selColorInNightMode : selColorInDayMode;
        mSelectionBlendMode = isNightMode ? PorterDuff.Mode.SCREEN : PorterDuff.Mode.MULTIPLY;
    }
    ...
    public void clear() {
        stop();
        getSelections().clear(mPdfViewCtrl);
    ...
    public void onDraw(Canvas canvas, Matrix tfm) {
        ...
        for (int pageNum : mPdfViewCtrl.getVisiblePagesInTransition()) { // Make and/or draw highlight paths for visible pages
            ...
            if (pgHlts != null && pgHlts.mLoaded) { // draw paths if not zooming
                ArrayList<QuadPath> quadPaths = pgHlts.mQuadPaths;
                for (QuadPath quadPath : quadPaths) {
                    ...
                    if (drawQuad) {
                        if (mPdfViewCtrl.isMaintainZoomEnabled()) {
                            canvas.save();
                            try {
                                int dx = (mPdfViewCtrl.isCurrentSlidingCanvas(pageNum) ? 0 : mPdfViewCtrl.getSlidingScrollX()) - mPdfViewCtrl.getScrollXOffsetInTools(pageNum);
        ...
        for (int pageNum : mPdfViewCtrl.getVisiblePagesInTransition()) { // Make and/or draw search paths for visible pages
            ...
            if (quadPaths != null) { // draw paths if not zooming
                for (QuadPath quadPath : quadPaths) {
                    ...
                    getSelections().createAndAddSelectionView(mPdfViewCtrl, quadPath.getRect(), pageNum, mSelectionColor, mSelectionBlendMode);
                }
            }
        }
    }
    ...
    private static final class QuadPath {
        ...
        Rect getRect() {
            Rect rect = null;
            try {
                rect = new Rect(mQuads[0], mQuads[1], mQuads[4], mQuads[5]);
                rect.normalize();
            } catch (PDFNetException e) {
                e.printStackTrace();
            }
    ...
    private TextSearchSelections getSelections() {
        return ((ToolManager) mPdfViewCtrl.getToolManager()).mTextSearchSelections;
    }

...
public class AnnotEditAdvancedShape extends AnnotEdit {
    ...
    protected void setCtrlPts(boolean resetAnnotView) {
        super.setCtrlPts(resetAnnotView);
        ...
        try {
            if (mPoly != null) {
                ...
                // adds content rect to vertices
                int xOffset = mPdfViewCtrl.getScrollX();
                int yOffset = mPdfViewCtrl.getScrollY();
                for (int i = 0; i < mCtrlPts.length; i++) {
                    PointF p = mCtrlPts[i];
                    vertices[i] = new PointF(p.x - xOffset, p.y - yOffset);
                }
        ...
        if (mAnnotView != null && mAnnotView.getDrawingView() != null) {
            mAnnotView.setVertices(vertices);
        }
    }
    ...
    protected void updateCtrlPts(boolean translate,
        ...
        if (mEffCtrlPtId == e_moving) {
            ...
            if (mAnnotView != null && mAnnotView.getDrawingView() != null) {
                mAnnotView.updateVertices(mEffCtrlPtId,
    ...
    protected void updateAnnot()
        ...
        RectF newAnnotScreenRect = getNewAnnotScreenPosition();
        newAnnotScreenRect.left -= mCtrlRadius;
        newAnnotScreenRect.top -= mCtrlRadius;
        newAnnotScreenRect.right = mCtrlRadius;
        newAnnotScreenRect.bottom = mCtrlRadius;

        float sx = mPdfViewCtrl.getScrollX();
        float sy = mPdfViewCtrl.getScrollY();
        if (mEffCtrlPtId == e_moving) {
            float deltaX = newAnnotScreenRect.left - mBBoxOnDown.left;
            float deltaY = newAnnotScreenRect.top - mBBoxOnDown.top;
            float x, y;
            double[] pts;
            boolean isContinuous = mPdfViewCtrl.isContinuousPagePresentationMode(mPdfViewCtrl.getPagePresentationMode());
            if (mPoly != null) {
                for (int i = 0; i < CTRL_PTS_CNT; ++i) {
                    Point pagePoint = mPoly.getVertex(i);
                    if (isContinuous) {
                        pts = mPdfViewCtrl.convPagePtToScreenPt(pagePoint.x, pagePoint.y, prevPage);
                        x = (float) pts[0] + sx;
                        y = (float) pts[1] + sy;
                    } else {
                        pts = mPdfViewCtrl.convPagePtToHorizontalScrollingPt(pagePoint.x, pagePoint.y, prevPage);
                        x = (float) pts[0];
                        y = (float) pts[1];
                    }
        ...
        RulerItem rulerItem = MeasureUtils.getRulerItemFromAnnot(mPoly);
        ArrayList<Point> points = AnnotUtils.getPolyVertices(mPoly);
        if (null != rulerItem && null != points) {
            if (mPoly.getType() == Annot.e_Polyline) {
                PerimeterMeasureCreate.adjustContents(mPoly, rulerItem, points);
            } else if (mPoly.getType() == Annot.e_Polygon) {
                AreaMeasureCreate.adjustContents(mPoly, rulerItem, points);
            }
        }
    ...
    /**
     * @return The new annotation position in screen space
     */
    private RectF getNewAnnotScreenPosition() {
        ...
        return snapAnnotScreenRectToPage(mBBox);
    }
    ...
    protected void updateAnnotView() {
        if (mAnnotView != null) {
            ...
            mAnnotView.setAnnotRect(new android.graphics.RectF(x1, y1, x2, y2));
            int xOffset = mPdfViewCtrl.getScrollX();
            int yOffset = mPdfViewCtrl.getScrollY();
            ...
            for (int i = 0; i < mCtrlPts.length; i++) {
                PointF p = mCtrlPts[i];
                if (p != null) {
                    mAnnotView.updateVertices(i, new PointF(p.x - xOffset, p.y - yOffset));
                }
            }

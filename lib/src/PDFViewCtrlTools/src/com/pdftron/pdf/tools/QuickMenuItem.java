...
/**
 * the <code>MenuItem</code> inside {@link QuickMenu}
 */
...
public class QuickMenuItem implements MenuItem, View.OnClickListener {
    ...
    public Button createTextButton() {
        ...
        if (!Utils.isNullOrEmpty(mTitle)) {
            String title = mAllCaps ? mTitle.toUpperCase() : mTitle;
...
//            setAlpha(0.87f);
    ...
    public View createSubMenuView() {
        ...
        View rootView = inflater.inflate(R.layout.quick_menu_submenu_item, null);
        TextView text = rootView.findViewById(R.id.submenu_item_text);
        ImageView icon = rootView.findViewById(R.id.submenu_item_image);
        if (!Utils.isNullOrEmpty(mTitle)) {
            String title = mAllCaps ? mTitle.toUpperCase() : mTitle;
            text.setText(title);
        }
        if (Utils.isRtlLayout(mContext)) {
        ...
        // Need to set layout params here otherwise layout will not be the correct size
        rootView.setLayoutParams(
                new RelativeLayout.LayoutParams((int) Utils.convDp2Pix(mContext, 120), (int) Utils.convDp2Pix(mContext, 48)));
        // Load style attributes to the text and icons
        loadAttributes(text);
        loadAttributes(icon);
        // Don't set a background for these items
    ...
    private void loadAttributes(View v) {
        ...
        try {
            mAllCaps = a.getBoolean(R.styleable.QuickMenuItem_android_textAllCaps, true);
        } finally {
            a.recycle();
        }
    }

    static void loadAttributes(@NonNull Context mContext, View v) {
        TypedArray a = mContext.obtainStyledAttributes(null, R.styleable.QuickMenuItem, R.attr.quick_menu_item, R.style.QuickMenuButton);
        try {
            // tint
            ...
            if (v instanceof ImageView) {
                ((ImageView) v).setColorFilter(tintColor);
                opacity = a.getFloat(R.styleable.QuickMenuItem_icon_alpha, 0.54f);
            } else if (v instanceof TextView) {
                ((TextView) v).setTextColor(tintColor);
     ...
     * <code>Menu</code> for the menu types that support icons.
     *
     ...
     * current tint mode, which is <code>PorterDuff.Mode.SRC_IN</code> by default.
     * <p>
     ...
     * tint mode using
     * <code>Drawable.setTintList(ColorStateList)</code>.
     *
     ...
     * <code>PorterDuff.Mode.SRC_IN</code>.
     *
     ...
     * in your application, you have to apply the tinting explicitly on the <code>Drawable</code>
    ...
    public MenuItem setEnabled(boolean enabled) {
        if (mButton != null) {
            ...
            if (enabled)  {
                loadAttributes(mButton);
            } else {
                mButton.setAlpha(0.1f);
            }
        }
        mIsEnable = enabled;
        return this;

...
public class UndoRedoManager implements
    ...
    final static String JSON_ACTION = "Action";
    ...
    // collab related
    final static String JSON_CHANGE = "change";
    ...
    private final static String ACTION_EVENT_REDACTION = "redaction";
    private final static String ACTION_EVENT_REPLACER = "replacer";
    private final static String ACTION_EVENT_PAGES_LABELS = "page_label_changed";
    private final static String ACTION_EVENT_MODIFY_OUTLINE = "modify_outline";

    // Used for work around when undo/redo-ing inks
    @Nullable
    ...
    public UndoRedoManager(@NonNull ToolManager toolManager) {
        ...
        mToolManager.addPdfTextModificationListener(this);
        mToolManager.addPdfOutlineModificationListener(this);
    ...
    public String undo(int locationId, boolean sendEvent) {
        ...
        // If mEditToolbarImpl not null, then we handle that first
        if (mEditToolbarImpl != null && mEditToolbarImpl.canUndo()) {
            mEditToolbarImpl.onUndoSelected();
            return "";
        } else {
    ...
    public String redo(int locationId, boolean sendEvent) {
        ...
        // If mEditToolbarImpl not null, then we handle that first
        if (mEditToolbarImpl != null && mEditToolbarImpl.canRedo()) {
            mEditToolbarImpl.onRedoSelected();
            return "";
        } else {
    ...
    private String performUndoRedo(boolean isUndo, int location, boolean sendEvent) {
        ...
        if (tool instanceof TextSelect) {
            // if text is selected, we want to deselect
            tool.onClose();
    ...
    public void onPageLabelsChanged() {
        ...
        try {
            JSONObject jsonObj = new JSONObject();
            if (mContext != null) {
                String strPgLabelsChanged = mContext.getResources().getString(R.string.undo_redo_page_labels);
                jsonObj.put(JSON_ACTION, strPgLabelsChanged);
            }
            jsonObj.put(JSON_ACTION_EVENT, ACTION_EVENT_PAGES_LABELS);
    ...
    public void onPdfTextChanged() {
        ...
        try {
            JSONObject jsonObj = new JSONObject();
            if (mContext != null) {
                String replacer = mContext.getResources().getString(R.string.tools_qm_edit);
                jsonObj.put(JSON_ACTION, replacer);
            }
            jsonObj.put(JSON_ACTION_EVENT, ACTION_EVENT_REPLACER);
    ...
    public void onOutlineChanged() {
        ...
        try {
            JSONObject jsonObj = new JSONObject();
            if (mContext != null) {
                String outline = mContext.getResources().getString(R.string.undo_redo_outline_modify);
                jsonObj.put(JSON_ACTION, outline);
            }
            jsonObj.put(ACTION_EVENT_MODIFY_OUTLINE, ACTION_EVENT_MODIFY_OUTLINE);
     ...
     * Preferably call this only from {@link ToolManager#raiseBookmarkModified(List)}
     */
    @Override
    ...
    public void onRedaction(@Nullable JSONObject jsonObj) {
        if (!mPdfViewCtrl.isUndoRedoEnabled()) {
            return;
        }
        ...
        try {
            ...
            if (mContext != null) {
                String redaction = mContext.getResources().getString(R.string.undo_redo_redaction);
                jsonObj.put(JSON_ACTION, redaction);
            }
            jsonObj.put(JSON_ACTION_EVENT, ACTION_EVENT_REDACTION);
    ...
    public boolean canUndo() {
        // If mEditToolbarImpl not null, then we check that first
        if (mEditToolbarImpl != null && mEditToolbarImpl.canUndo()) {
            return true;
        } else {
    ...
    public String getNextUndoAction() {
        ...
        if (mContext != null && mEditToolbarImpl != null && mEditToolbarImpl.canUndo()) {
            return mContext.getResources().getString(R.string.undo);
        }
        ...
        try {
            if (!mPdfViewCtrl.canUndo()) {
                // if core already tells us no more undo, no need to check further
                return "";
            }
            ...
            } else if (jsonObj.has(JSON_CHANGE)) {
    ...
    public String getNextRedoAction() {
        ...
        if (mContext != null && mEditToolbarImpl != null && mEditToolbarImpl.canRedo()) {
            return mContext.getResources().getString(R.string.redo);
        }

        removeUnsafeUndoRedoInfo(false);
        String info = null;
        try {
            if (!mPdfViewCtrl.canRedo()) {
                // if core already tells us no more redo, no need to check further
                return "";
            }
            ...
            if (jsonObj.has(JSON_ACTION)) {
                ...
                // collaboration remote change
                result = mContext.getResources().getString(R.string.redo);
            }
        } catch (Exception e) {
            if (info == null || !info.equals(JSON_STATE_NOT_FOUND)) {
     ...
     *
     * @return True if the information attached to an undo is related to editing page labels; False otherwise
     */
    public static boolean isEditPageLabelsAction(final Context context, final String info) {
        if (context != null && !Utils.isNullOrEmpty(info)) {
            try {
                JSONObject jsonObj = new JSONObject(info);
                if (jsonObj.has(JSON_ACTION_EVENT)) {
                    String action = jsonObj.getString(JSON_ACTION_EVENT);
                    if (ACTION_EVENT_PAGES_LABELS.equals(action)) {
                        return true;
                    }
                }
            } catch (Exception e) {
     ...
     *
     * @return True if the information attached to an undo is related to editing content; False otherwise
     */
    public static boolean isEditContentAction(final Context context, final String info) {
        if (context != null && !Utils.isNullOrEmpty(info)) {
            try {
                JSONObject jsonObj = new JSONObject(info);
                if (jsonObj.has(JSON_ACTION_EVENT)) {
                    String action = jsonObj.getString(JSON_ACTION_EVENT);
                    if (ACTION_EVENT_REPLACER.equals(action)) { // page label changes can be considered as page edit
                        return true;
                    }
                }
            } catch (Exception e) {
            ...
            try {
                ...
                if (jsonObj.has(JSON_ACTION_EVENT)) {
                    ...
                    if (ACTION_EVENT_ADD_PAGES.equals(action) ||
                            ...
                            ACTION_EVENT_PAGES_LABELS.equals(action)) { // page label changes can be considered as page edit
                        return true;
                    }
                }
            } catch (Exception e) {
    ...
    public static void jumpToUndoRedo(PDFViewCtrl pdfViewCtrl, final String info, boolean isUndo) {
        ...
        if (isEditPageAction(context, info)) {
            ...
            } else if (isRotatePagesAction(context, info) || isEditPageLabelsAction(context, info)) {
                List<Integer> pageList = getPageList(info);
                if (pageList.size() != 0) {
    ...
    public JSONObject getAnnotSnapshot(final Annot annot, int page) {
        Map<Annot, Integer> annots = new HashMap<>(1);
        annots.put(annot, page);
        return prepareAnnotSnapshot(annots, null);
    }
    ...
    private static List<Integer> getAnnotPageNumbers(final String info) {
        ...
        if (!Utils.isNullOrEmpty(info)) {
            try {
                ...
                if (!Utils.isNullOrEmpty(annotInfo)) {
    ...
    private static boolean isValidAction(final Context context, final String action) {
        ...
        if (context != null && !Utils.isNullOrEmpty(action)) {
            ...
            String strRedactionAction = context.getResources().getString(R.string.undo_redo_redaction);
            String strEditAction = context.getResources().getString(R.string.tools_qm_edit);
            ...
            return action.equals(strBookmarkModify) || action.equals(strCropPages) ||
                    ...
                    action.contains(strAnnotAction) || action.equals(strRedactionAction) ||
                    action.contains(strEditAction);
        }
    ...
    private void updatePageLayout(final String info) {
        ...
        if (isEditPageAction(mContext, info)) {
            try {
                mPdfViewCtrl.docLockRead(() -> mPdfViewCtrl.updatePageLayout());
            } catch (Exception e) {
                AnalyticsHandlerAdapter.getInstance().sendException(e);
            }
        }
    ...
    // Tell listeners that undo redo state changed
    public void notifyUndoRedoStateChange() {
        for (UndoRedoStateChangeListener listener : mUndoRedoStateChangeListeners) {
    ...
    public void clearUndoRedoStateChangeListener() {
        mUndoRedoStateChangeListeners.clear();
    }
     ...
     */
    public void removeUndoRedoStateChangeListener(@NonNull UndoRedoStateChangeListener listener) {
        mUndoRedoStateChangeListeners.remove(listener);
    }
     ...
     */
    public void addUndoRedoStateChangeListener(@NonNull UndoRedoStateChangeListener listener) {
        mUndoRedoStateChangeListeners.add(listener);
    }

    /**
     * Listener that gets notfied when the undo/redo stack has changed
     */
    ...
    public void setEditToolbarImpl(@Nullable EditToolbarImpl editToolbarImpl) {
        mEditToolbarImpl = editToolbarImpl;
    }
}

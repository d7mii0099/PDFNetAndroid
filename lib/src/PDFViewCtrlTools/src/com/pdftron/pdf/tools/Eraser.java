...
public class Eraser extends SimpleShapeCreate {
    ...
    private HashMap<Ink, Rect> mErasedInks; // contains only inks that have been erased and the region to update
    ...
    public enum InkEraserMode {
        ...
        private InkEraserMode(@StringRes int labelRes) {
            this.mLabelRes = labelRes;
        }

        /**
         * Returns the {@link InkEraserMode} related to a specific label
         *
         * @param context          the context used to inflate related string resources
         * @param eraserModeString the string used to find the {@link InkEraserMode}
         * @return the {@link InkEraserMode} related to a specific string
         */
        @Nullable
        public static InkEraserMode fromLabel(@NonNull Context context, @NonNull String eraserModeString) {
            for (InkEraserMode mode : InkEraserMode.values()) {
                if (context.getResources().getString(mode.mLabelRes).equals(eraserModeString)) {
                    return mode;
                }
            }
            return null;
    ...
    public Eraser(@NonNull PDFViewCtrl ctrl) {
        this(ctrl, EraserType.HYBRID_ERASER, InkEraserMode.PIXEL);
    }
    ...
    public Eraser(@NonNull PDFViewCtrl ctrl, @NonNull EraserType eraserType) {
        this(ctrl, eraserType, InkEraserMode.PIXEL);
    }
    ...
    public Eraser(@NonNull PDFViewCtrl ctrl, @NonNull EraserType eraserType, @NonNull InkEraserMode inkEraserMode) {
        ...
        mEraserHalfThickness = 0.5f * settings.getFloat(getThicknessKey(getCreateAnnotType()), ToolStyleConfig.getInstance().getDefaultThickness(mPdfViewCtrl.getContext(), getCreateAnnotType()));
        String eraserTypeStr = settings.getString(getEraserTypeKey(getCreateAnnotType()), null);
        if (eraserTypeStr != null) {
            mEraserType = EraserType.valueOf(eraserTypeStr);
        }
        String inkEraserModeStr = settings.getString(getInkEraserModeKey(getCreateAnnotType()), null);
        if (inkEraserModeStr != null) {
            mInkEraserMode = InkEraserMode.valueOf(inkEraserModeStr);
        }
    ...
    public void setupAnnotProperty(AnnotStyle annotStyle) {
        ...
        mEraserType = annotStyle.getEraserType();
        mInkEraserMode = annotStyle.getInkEraserMode();

        SharedPreferences settings = Tool.getToolPreferences(mPdfViewCtrl.getContext());
        SharedPreferences.Editor editor = settings.edit();
        editor.putFloat(getThicknessKey(getCreateAnnotType()), thickness);
        editor.putString(getEraserTypeKey(getCreateAnnotType()), mEraserType.name());
        editor.putString(getInkEraserModeKey(getCreateAnnotType()), mInkEraserMode.name());
        editor.apply();
    }
    ...
    public boolean onMove(MotionEvent e1, MotionEvent e2, float x_dist, float y_dist) {
        ...
        if (mEraserType == EraserType.ANNOTATION_ERASER ||
                mEraserType == EraserType.HYBRID_ERASER ||
            ...
            // todo bfung: investigate locking in onMove might be bad
            ...
            try {
                ...
                switch (mEraserType) {
                    case INK_ERASER:
                        handleMoveInk(page, pdfPt1, pdfPt2);
                        break;
                    case ANNOTATION_ERASER:
                        handleMoveAnnot(false);
                        break;
                    case HYBRID_ERASER:
                        handleMoveInk(page, pdfPt1, pdfPt2);
                        handleMoveAnnot(true);
                        break;
                }
            } catch (Exception e) {
    ...
    private void safeSetNextToolMode() {
        if (mForceSameNextToolMode) {
            if (mCurrentDefaultToolMode == ToolMode.INK_CREATE ||
                    mCurrentDefaultToolMode == ToolMode.SMART_PEN_INK) {
    ...
    private void handleMoveInk(Page page, Point pdfPt1, Point pdfPt2) throws PDFNetException {
        ...
        for (int i = annot_num - 1; i >= 0; --i) {
            ...
            if (annot.getType() == Annot.e_Ink) {
                ...
                Rect updateRegion = ink.getContentRect();
                switch (mInkEraserMode) {
                    case PIXEL:
                        erased = ink.erase(pdfPt1, pdfPt2, mEraserHalfThickness);
                        break;
                    case STROKE:
                        erased = ink.erasePaths(pdfPt1, pdfPt2, mEraserHalfThickness);
                }
                if (erased) {
                    if (!mErasedInks.containsKey(ink)) {
                        mErasedInks.put(ink, updateRegion);
                        mDoUpdate = true;
                    }
                }
            }
    ...
    private void handleUpInk(Point pdfPt1) throws PDFNetException {
        ...
        for (int i = annot_num - 1; i >= 0; --i) {
            ...
            if (annot.getType() == Annot.e_Ink) {
                ...
                switch (mInkEraserMode) {
                    case PIXEL:
                        erased = ink.erase(pdfPt1, pdfPt1, mEraserHalfThickness);
                        break;
                    case STROKE:
                        erased = ink.erasePaths(pdfPt1, pdfPt1, mEraserHalfThickness);
                        break;
                }
                if (erased) {
                    if (!mErasedInks.containsKey(ink)) {
    ...
    protected boolean commitAnnotation() {
        ...
        try {
            ...
            switch (mEraserType) {
                case INK_ERASER:
                    needsRender = commitInk(page);
                    break;
                case ANNOTATION_ERASER:
                    needsRender = commitAnnot(page);
                    break;
                case HYBRID_ERASER:
                    boolean inkResult = commitInk(page);
                    boolean annotResult = commitAnnot(page);
                    needsRender = inkResult || annotResult;
                    break;
            }
        } catch (Exception e) {
            AnalyticsHandlerAdapter.getInstance().sendException(e);
    ...
    private boolean commitInk(Page page) throws PDFNetException {
        ...
        HashMap<Annot, Integer> inksToRemove = new HashMap<>();
        HashMap<Annot, Integer> inksRemoved = new HashMap<>();
        HashMap<Annot, Integer> inksToModify = new HashMap<>();

        // Handle normal inks. We do not need to erase normal inks and they were erased in onMove
        for (Ink ink : mErasedInks.keySet()) {
            if (ink.getPathCount() != 0) {
                ...
                if (shouldRemove) {
                    inksToRemove.put(ink, mAnnotPageNum);
                } else {
                    inksToModify.put(ink, mAnnotPageNum);
                }
            } else {
                inksToRemove.put(ink, mAnnotPageNum);
            }
        ...
        // Remove all inks that were completely erased
        if (!inksToRemove.isEmpty()) {
            raiseAnnotationPreRemoveEvent(inksToRemove);
            for (Annot ink : inksToRemove.keySet()) {
                Rect rect = null;
                if (ink instanceof Ink) {
                    rect = mErasedInks.get(ink);
                }
                ink = AnnotUtils.safeDeleteAnnot(mPdfViewCtrl.getDoc(), page, ink);
                inksRemoved.put(ink, mAnnotPageNum);
                if (rect != null) {
                    PDFViewCtrUpdateRect(rect);
                }
                needsRender = true;
            }
            raiseAnnotationRemovedEvent(inksRemoved);
        }

        // Now modify all inks that were partially erased. Since they've already been erased we'll just refresh appearance
        if (!inksToModify.isEmpty()) {
            raiseAnnotationPreModifyEvent(inksToModify);
            for (Annot ink : inksToModify.keySet()) {
                if (PressureInkUtils.isPressureSensitive(ink)) {
                    // Update the ink rect since we might be missing some strokes
                    Annot.BorderStyle borderStyle = ink.getBorderStyle();
                    float width = (float) borderStyle.getWidth();
                    List<List<PointF>> inkList = FreehandCreate.createStrokeListFromArrayObj(ink.getSDFObj().findObj(AnnotUtils.KEY_INK_LIST));
                    Rect rect = PressureInkUtils.getInkItemBBox(inkList, width, 0, null, false);
                    if (rect != null) {
                        ink.setRect(rect);
                    }
                ...
                if (ink instanceof Ink) {
                    Rect rect = mErasedInks.get((Ink) ink);
                    if (rect != null) {
                        PDFViewCtrUpdateRect(rect);
                    }
                }
            ...
            raiseAnnotationModifiedEvent(inksToModify);
        }

        return needsRender;
    }

    // Must be called in read lock
    private void PDFViewCtrUpdateRect(@NonNull Rect updateRegionInPagePt) throws PDFNetException {
        // Update viewer
        double[] screenPt1 = mPdfViewCtrl.convPagePtToScreenPt(updateRegionInPagePt.getX1(), updateRegionInPagePt.getY1(), mDownPageNum);
        double[] screenPt2 = mPdfViewCtrl.convPagePtToScreenPt(updateRegionInPagePt.getX2(), updateRegionInPagePt.getY2(), mDownPageNum);
    ...
    private boolean commitAnnot(Page page) throws PDFNetException {
        boolean needsRender = false;
        HashMap<Annot, Integer> annotsToRemove = new HashMap<>();
        HashMap<Annot, Integer> annotsPostRemove = new HashMap<>();
        for (Annot annot : mAnnotList) {
            annotsToRemove.put(annot, mAnnotPageNum);
        }
        raiseAnnotationPreRemoveEvent(annotsToRemove);
        for (Annot annot : mAnnotList) {
            if (!ToolConfig.getInstance().isAnnotEditingDisabled(annot.getType())) {
                annot = AnnotUtils.safeDeleteAnnotAndUpdate(mPdfViewCtrl, page, annot, mAnnotPageNum);
                needsRender = true;
                annotsPostRemove.put(annot, mAnnotPageNum);
            }
        }
        raiseAnnotationRemovedEvent(annotsPostRemove);
        return needsRender;
    }

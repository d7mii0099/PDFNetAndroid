...
public class QuickMenu extends RelativeLayout implements View.OnClickListener, MenuItem.OnMenuItemClickListener {
     ...
     *
     * @param menuEntries Menu resource to remove
     * @return true any menu entries were removed as a result of the call
     */
    public boolean removeMenuEntries(@NonNull List<QuickMenuItem> menuEntries) {
        ...
        for (QuickMenuItem entry : menuEntries) {
            switch (entry.getDisplayMode()) {
                case QuickMenuItem.FIRST_ROW_MENU:
                    result = mFirstMenuItems.remove(entry);
                    break;
                case QuickMenuItem.SECOND_ROW_MENU:
                    // it is possible that second row will overflow
                    if (mSecondMenuItems.contains(entry)) {
                        result = mSecondMenuItems.remove(entry);
                    } else {
                        result = mOverflowMenuItems.remove(entry);
                    }
                    break;
                default:
                    result = mOverflowMenuItems.remove(entry);
        ...
        // do not show divider if no first row
        if (mFirstMenuItems.isEmpty()) {
            setDividerVisibility(View.GONE);
        }
        // do not show overflow if no overflow items
    ...
    /**
     * Add existing menu entries to quick menu, if exceed max row size, menu entries
    ...
    protected void initView() {
        ...
        mOverflowLayoutRoot = mMenuView.findViewById(R.id.overflow_group_root);
        ...
        QuickMenuItem.loadAttributes(mParentView.getContext(), mBackButton);
        if (Utils.isNougat()) {
...
//            view.setOnClickListener(this);
        ...
        mOverflowLayoutRoot.measure(MeasureSpec.UNSPECIFIED, MeasureSpec.UNSPECIFIED);
        mOverflowWidth = mOverflowLayoutRoot.getMeasuredWidth();
        mOverflowHeight = mOverflowLayoutRoot.getMeasuredHeight();

        // height adjustment for overflow
        int availableHeight = mToolManager.getPDFViewCtrl().getMeasuredHeight();
        if (mOverflowHeight > availableHeight) {
            mOverflowHeight = availableHeight - mPaddingOffsetPx;
        }
    ...
    protected void initAnimation() {
        ...
        mMain2OverflowAnim.setInterpolator(new DecelerateInterpolator());
        mOverflow2MainAnim.setInterpolator(new DecelerateInterpolator());
        ...
        mMain2OverflowAnim.setAnimationListener(new Animation.AnimationListener() {
            ...
            public void onAnimationEnd(Animation animation) {
                ...
                mOverflowLayoutRoot.setVisibility(VISIBLE);
                mOverflowLayoutRoot.startAnimation(mFadeInAnim);
    ...
    public void requestLocation() {
        ...
        if (Utils.isRtlLayout(getContext())) {
            mOverflowLayoutRoot.layout(margin, margin, mOverflowWidth + margin, mOverflowHeight + margin);
        }
     ...
     */
    public void show() {
        if (mToolManager != null && mToolManager.onShowQuickMenu(this, mAnnot)) {
            return;
        }
        ...
        post(new Runnable() {
            ...
            public void run() {
                ...
                if (mToolManager != null) {
                    mToolManager.onQuickMenuLocationRequested();
                }
            }
        });
    ...
    public void hideOverflowAnim() {
        Animation fadeOutAnimation = AnimationUtils.loadAnimation(getContext(), R.anim.controls_quickmenu_hide);
        fadeOutAnimation.setAnimationListener(new FadeOutAnimListener(mOverflowLayoutRoot, mBgView, mOverflow2MainAnim));
        mOverflowLayoutRoot.startAnimation(fadeOutAnimation);
    }
    ...
    public boolean onMenuItemClick(MenuItem item) {
        ...
        if (item.hasSubMenu()) {
            ...
            addMenuEntries(subMenu.getMenuItems(), Tool.QM_MAX_ROW_SIZE);
            mMainLayout.setVisibility(VISIBLE);
            mOverflowLayoutRoot.setVisibility(INVISIBLE);
            mParentView.removeView(this);
            show();
            return true;
        }
        ...
        if (!item.equals(mOverflowMenuItem)) {
            ...
            if (id == R.id.qm_floating_sig) {
                ...
                action = "shapes";
                nextAction = AnalyticsHandlerAdapter.NEXT_ACTION_SHAPE_RULER;
            } else if (id == R.id.qm_callout) {
                action = "shapes";
                nextAction = AnalyticsHandlerAdapter.NEXT_ACTION_SHAPE_CALLOUT;
            } else if (id == R.id.qm_form_text) {
                action = "forms";
                nextAction = AnalyticsHandlerAdapter.NEXT_ACTION_FORM_TEXT_FIELD;
            } else if (id == R.id.qm_form_check_box) {
                action = "forms";
                nextAction = AnalyticsHandlerAdapter.NEXT_ACTION_FORM_CHECKBOX;
            } else if (id == R.id.qm_form_signature) {
                action = "forms";
                nextAction = AnalyticsHandlerAdapter.NEXT_ACTION_FORM_SIGNATURE;
            }

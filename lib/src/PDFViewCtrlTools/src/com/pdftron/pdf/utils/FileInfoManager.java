...
public class FileInfoManager {
    ...
    private void save(@NonNull SharedPreferences prefs, @NonNull List<FileInfo> files) {
        ...
        for (FileInfo fileInfo : files) {
            ...
            serializedDoc = gsonFilePathFallback(serializedDoc, fileInfo);
    ...
    private String gsonFilePathFallback(String serializedDoc, @NonNull FileInfo fileInfo) {
        String result = serializedDoc;
        try {
            JSONObject jsonObject = new JSONObject(serializedDoc);
            JSONObject fileObject = jsonObject.optJSONObject(FileInfo.VAR_FILE);
            if (fileObject == null || Utils.isNullOrEmpty(fileObject.optString(FileInfo.VAR_PATH))) {
                JSONObject path = new JSONObject();
                path.put(FileInfo.VAR_PATH, fileInfo.getAbsolutePath());
                jsonObject.put(FileInfo.VAR_FILE, path);

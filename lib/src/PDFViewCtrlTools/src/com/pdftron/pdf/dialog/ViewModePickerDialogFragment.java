...
public class ViewModePickerDialogFragment extends DialogFragment {
    ...
    public enum ViewModePickerItems {
        ITEM_ID_CONTINUOUS(ViewModePickerDialogFragment.ITEM_ID_CONTINUOUS),
        ITEM_ID_TEXT_SIZE(ViewModePickerDialogFragment.ITEM_ID_TEXT_SIZE),
        ITEM_ID_ROTATION(ViewModePickerDialogFragment.ITEM_ID_ROTATION),
        ITEM_ID_USERCROP(ViewModePickerDialogFragment.ITEM_ID_USERCROP),
        ITEM_ID_COLORMODE(ViewModePickerDialogFragment.ITEM_ID_COLORMODE),
        ITEM_ID_READING_MODE(ViewModePickerDialogFragment.ITEM_ID_READING_MODE),
        ITEM_ID_REFLOW(ViewModePickerDialogFragment.ITEM_ID_REFLOW),
        ITEM_ID_FACING_COVER(ViewModePickerDialogFragment.ITEM_ID_FACING_COVER);
     ...
     * {@link #ITEM_ID_READING_MODE}, {@link #ITEM_ID_REFLOW}
     */
    @IntDef({
            ITEM_ID_CONTINUOUS, ITEM_ID_TEXT_SIZE, ITEM_ID_ROTATION,
            ITEM_ID_USERCROP, ITEM_ID_RTLMODE, ITEM_ID_BLANK, ITEM_ID_COLORMODE, ITEM_ID_READING_MODE, ITEM_ID_REFLOW
    })
    ...
    /**
     * Reflow mode control item
     */
    protected static final int ITEM_ID_REFLOW = 109;

    /**
     * Facing cover control item
     */
     ...
     */
    @IntDef({
            CONTROL_TYPE_RADIO, CONTROL_TYPE_SWITCH, CONTROL_TYPE_SIZE,
            CONTROL_TYPE_NONE
    })
    @Retention(RetentionPolicy.SOURCE)
     ...
     * @param hiddenItems     view mode items to hide
     * @return view mode picker dialog fragment
     */
    public static ViewModePickerDialogFragment newInstance(PDFViewCtrl.PagePresentationMode currentViewMode,
            boolean isRTLMode,
            boolean isReflowMode,
            int reflowTextSize,
            @Nullable ArrayList<Integer> hiddenItems) {
        ...
        if (hiddenItems != null) {
            args.putIntegerArrayList(BUNDLE_HIDDEN_ITEMS, hiddenItems);
        }
        f.setArguments(args);
        return f;
    }
    ...
    public static ViewModePickerDialogFragment newInstance(PDFViewCtrl.PagePresentationMode currentViewMode,
        ...
        return newInstance(currentViewMode, isRTLMode, isReflowMode, reflowTextSize, null);
    }
    ...
    protected void populateOptionsList() {
        ...
        addItem(mViewModeOptionsList,
                createItem(ITEM_ID_CONTINUOUS,
                        res.getDrawable(R.drawable.ic_view_mode_continuous_black_24dp),
                        getString(R.string.pref_viewmode_scrolling_direction),
                        CONTROL_TYPE_SWITCH));

        addItem(mViewModeOptionsList,
                createItem(ITEM_ID_TEXT_SIZE,
                        res.getDrawable(R.drawable.ic_font_size_black_24dp),
                        getString(R.string.pref_viewmode_reflow_text_size),
                        CONTROL_TYPE_SIZE));

        addItem(mViewModeOptionsList,
                createItem(ITEM_ID_COLORMODE,
                        null,
                        null,
                        CONTROL_TYPE_RADIO));
        addItem(mViewModeOptionsList,
                createItem(ITEM_ID_READING_MODE,
                        res.getDrawable(R.drawable.ic_reading_mode_settings),
                        getString(R.string.reading_mode_settings),
                        CONTROL_TYPE_NONE));

        if (PdfViewCtrlSettingsManager.hasRtlModeOption(context)) {
            addItem(mViewModeOptionsList, createItem(ITEM_ID_RTLMODE,
                    res.getDrawable(R.drawable.rtl),
                    getString(R.string.pref_viewmode_rtl_mode),
                    CONTROL_TYPE_SWITCH));
        }

        if (mViewModeOptionsList.size() > 0) {
            addItem(mViewModeOptionsList,
                    createItem(ITEM_ID_BLANK,
                            null,
                            null,
                            CONTROL_TYPE_NONE));
        ...
        addItem(list,
                createItem(ITEM_ID_ROTATION,
                        res.getDrawable(R.drawable.ic_rotate_right_black_24dp),
                        getString(R.string.pref_viewmode_rotation),
                        CONTROL_TYPE_NONE));

        addItem(list,
                createItem(ITEM_ID_USERCROP,
                        res.getDrawable(R.drawable.user_crop),
                        getString(R.string.pref_viewmode_user_crop),
                        CONTROL_TYPE_NONE));
    ...
    private void addItem(List<Map<String, Object>> list, Map<String, Object> item) {
        int id = (Integer) item.get(KEY_ITEM_ID);
        if (!mHiddenItems.contains(id)) {
            list.add(item);
        }
    }
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        mHiddenItems = getArguments().getIntegerArrayList(BUNDLE_HIDDEN_ITEMS);
    ...
    public Dialog onCreateDialog(Bundle savedInstanceState) {
        ...
        View reflowItem = mViewModeLayout.findViewById(R.id.fragment_view_mode_button_reflow);
        View facingCover = mViewModeLayout.findViewById(R.id.fragment_view_mode_button_cover);
        if (mHiddenItems.contains(ITEM_ID_REFLOW)) {
            reflowItem.setVisibility(View.GONE);
        }
        if (mHiddenItems.contains(ITEM_ID_FACING_COVER)) {
            facingCover.setVisibility(View.GONE);
        }
            ...
            public void onItemClick(AdapterView<?> parent, View view1, int position, long id) {
                switch ((int) id) {
                    ...
                    case ITEM_ID_USERCROP:
                        ...
                        break;
                    case ITEM_ID_READING_MODE:
                        if (mViewModePickerDialogListener != null) {
                            mViewModePickerDialogListener.onViewModeSelected(PdfViewCtrlSettingsManager.KEY_PREF_VIEWMODE_READING_SETTINGS_VALUE);
                        }
                        mHasEventAction = true;
                        AnalyticsHandlerAdapter.getInstance().sendEvent(AnalyticsHandlerAdapter.EVENT_VIEW_MODE,
                                AnalyticsParam.viewModeParam(AnalyticsHandlerAdapter.VIEW_MODE_READING_SETTINGS));
                        dismiss();
                        break;
                    default:
                        break;
        ...
        final ScrollView sView = view.findViewById(R.id.view_mode_scrollview);
        try {
    ...
    protected void setActiveColorMode(@IdRes int id) {
        ...
        if (id == R.id.item_view_mode_picker_customcolor_button) {
            ...
            frag.setStyle(DialogFragment.STYLE_NORMAL, getTheme());
    ...
    private void showReadingModeSettings() {
        ...
        Map<String, Object> readingMode = null;
        for (int i = 0; i < mViewModeOptionsList.size(); i++) {
            Map<String, Object> item = mViewModeOptionsList.get(i);
            if (item != null && (int) item.get(KEY_ITEM_ID) == ITEM_ID_READING_MODE) {
                readingMode = item;
            }
        }
        if (mIsReflowMode && readingMode == null) {
            addItem(mViewModeOptionsList, createItem(ITEM_ID_READING_MODE,
                    ContextCompat.getDrawable(activity, R.drawable.ic_reading_mode_settings),
                    getString(R.string.reading_mode_settings),
                    CONTROL_TYPE_NONE));
        } else if (!mIsReflowMode && readingMode != null) {
            mViewModeOptionsList.remove(readingMode);
        }
    }
    ...
    private void listSwapFirstSecondViewModeOptions() {
        if (mViewModeOptionsList.size() > 1) {
    ...
    private void setViewMode(boolean verticalScrolling) {
        int activeId = getActiveMode();
        if (mViewModeOptionsList.size() > 0 && activeId != R.id.fragment_view_mode_button_reflow
        ...
        if (verticalScrolling) {
            ...
            if (activeId == R.id.fragment_view_mode_button_single) {
                ...
                if (mViewModeOptionsList.size() > 0 && (int) mViewModeOptionsList.get(0).get(KEY_ITEM_ID) == ITEM_ID_CONTINUOUS) {
                    listSwapFirstSecondViewModeOptions();
                }
                if (mViewModePickerDialogListener != null) {
        ...
        public View getView(final int position, View convertView, @NonNull ViewGroup parent) {
            ...
            if (convertView == null || convertView.getTag() == null) {
                ...
                if (id == ITEM_ID_COLORMODE) {
                    ...
                    AppCompatImageButton customColorBtn = layoutView.findViewById(R.id.item_view_mode_picker_customcolor_button);
                    Utils.applySecondaryTextTintToButton(customColorBtn);
                    ...
                    applyTintList((androidx.appcompat.widget.AppCompatImageView) layoutView.findViewById(R.id.item_view_mode_picker_color_list_icon), mIconTintList);
